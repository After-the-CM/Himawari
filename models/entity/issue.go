...
var csrfVuln = Vuln{
	...
	Mandatory: "対策の必要な部分にワンタイムトークンの生成とチェック機能を実装しましょう。トークンによってリクエストの正当性を確認することができます。\n" +
		...
		"新規デバイスのログイン、パスワード変更などの重要な操作を行った際は、アカウントに登録されているメールアドレスに確認を行いましょう。\n" +
		"Refererヘッダの確認は行いましょう。\n",
	Insurance: "パスワード変更などの重要な操作を行った際は、アカウントに登録されているメールアドレスに確認を行いましょう。\n",
	Issues:    []Issue{},
}
...
var reflectedxssVuln = Vuln{
	...
	Insurance: "X-XSS-Protectionレスポンスヘッダを利用することでブラウザ機能により反射型XSSを無害な出力に変更できます。X-XSS-Protectionレスポンスヘッダを利用していれば利用者が無効化している場合でも上書きして有効にすることができます。\n" +
		"CSP(Contents Security Policy)の設定することでevalの禁止, インラインスクリプトを禁止にする等、スクリプトに対してより強い制約を設けることができます。\n" +
		"入力値の検証は行ったほうがよいです。この対策は限定的ですが、郵便番号など特定の文字の種類しか入力を求めていない箇所に要求を満たしていない入力がされた場合にエラーを表示し、再入力を求めるようにすることでスクリプトを入力できないようにしたり、入力値の長さを制限することができます。\n" +
		"上記の検証はサーバ側で行ったほうがよいです。クライアント上で入力値の検証を行ったとしても、proxyツールなどの使用により攻撃者は要求を満たしていない値の送信が可能です。これを防ぐために、サーバ側で適切な検証を行いましょう。\n" +
...
var storedxssVuln = Vuln{
	...
	Description: "持続型クロスサイト・スクリプティング(XSS)は、攻撃者によって仕掛けられた悪意のあるスクリプトをサイトに訪れたユーザのブラウザ上で実行させることができる脆弱性です。\n" +
		"持続型XSSの脆弱性が存在すると、攻撃者によって書き込まれた悪意のあるスクリプトがサイトのDB等に保存されます。攻撃用のスクリプトがDBに保存されているとWebアプリケーションにアクセスするたびにスクリプトが実行されてしまいます。\n",
	Mandatory: "要素内容/属性値の文字をエスケープして、属性値はダブルクォーテーションでくくりましょう。\n" +
		"(例. `<` を `&lt;` にする等、HTML特殊文字はHTMLエンティティに置き換えましょう。)\n" +
		"HTTPレスポンスヘッダのContent-Typeフィールドに文字コードを指定しましょう。文字コードの指定を省略している場合、Webアプリケーションとブラウザとの文字エンコーディングの差異でXSSの原因になりえます。\n",
	Insurance: "CSP(Contents Security Policy)の設定することでevalの禁止, インラインスクリプトを禁止にする等、スクリプトに対してより強い制約を設けることができます。\n" +
		"入力値の検証は行ったほうがよいです。この対策は限定的ですが、郵便番号など特定の文字の種類しか入力を求めていない箇所に要求を満たしていない入力がされた場合にエラーを表示し、再入力を求めるようにすることでスクリプトを入力できないようにしたり、入力値の長さを制限することができます。\n" +
		"上記の検証はサーバ側で行ったほうがよいです。クライアント上で入力値の検証を行ったとしても、proxyツールなどの使用により攻撃者は要求を満たしていない値の送信が可能です。これを防ぐために、サーバ側で適切な検証を行いましょう。\n" +
		"CookieにHttpOnly属性を付与するとCookieに保存されているセッションIDへのアクセスを防ぐことができます。",
	Issues: []Issue{},
...
var dirtraversalVuln = Vuln{
	CWE:         "CWE-22",
	Severity:    "High",
	Description: "ディレクトリ・トラバーサルは、攻撃者によって制限されたディレクトリ外の任意のファイルに対して閲覧をはじめとする、開発者の意図しない処理が行われる脆弱性です。\n",
	Mandatory: "Webサーバ内のファイル名を外部からのパラメータで指定する実装は避けてください。\n" +
		"ファイルを開く部分の実装は、固定のディレクトリを指定して、かつファイル名にディレクトリ名が含まれないようにしましょう。\n",
	Insurance: "Webサーバ内のファイルへのアクセス権限の設定は正しく管理するようにしましょう。Webサーバ側でアクセスを拒否することができます。\n" +
		"ファイル名の文字種を英数字に限定しておけば、ディレクトリ・トラバーサル攻撃時に利用される記号文字が使用できなくなります。\n",
...
var httpheaderiVuln = Vuln{
	...
	Insurance: "外部からの入力の改行コードはすべて削除するようにしましょう。改行コードを含む文字列を受け付ける必要がある場合はWebアプリケーションが正しく動作しなくなる場合があります。\n",
	Issues:    []Issue{},
}
