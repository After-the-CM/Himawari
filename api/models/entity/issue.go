...
var csrfVuln = Vuln{
	...
	Mandatory: "入力値が永続化されるような部分にワンタイムトークンの生成とチェック機能を実装しましょう。トークンによってリクエストの正当性を確認することができます。\n" +
...
var reflectedxssVuln = Vuln{
	...
	Insurance: "X-XSS-Protectionレスポンスヘッダを利用することでブラウザ機能により反射型XSSを無害な出力に変更できます。X-XSS-Protectionレスポンスヘッダを利用していれば利用者が無効化している場合でも上書きして有効にすることができます。\n" +
		...
		"上記の検証はサーバ側で行いましょう。クライアント上で入力値の検証を行ったとしても、プロキシツールなどの使用により攻撃者は要求を満たしていない値の送信が可能です。これを防ぐために、サーバ側で適切な検証を行いましょう。\n" +
		"CookieにHttpOnly属性を付与するとCookieに保存されている情報へのアクセスを防ぐことができます。",
	Issues: []Issue{},
}

var storedxssVuln = Vuln{
	Name:     "持続型クロスサイト・スクリプティング",
	CWE:      "CWE-79",
	Severity: "High",
...
var timebasedsqliVuln = Vuln{
	...
	Mandatory: "SQL文の組み立てはプリペアドステートメントもしくは静的プレースホルダで行うよう実装しましょう。\n" +
...
var errbasedsqliVuln = Vuln{
	...
	Mandatory: "SQL文の組み立てはプリペアドステートメントもしくは、静的プレースホルダで行うよう実装しましょう。\n" +
		"Webアプリケーションに渡されるパラメータでSQL文を直接指定しないようにしましょう。\n" +
		"文字列リテラル内で特別な意味を持つ記号文字「'」を「\\'」、「\\」を「\\\\」などに置き換えるエスケープ処理を行いましょう。",
...
var openredirectVuln = Vuln{
	...
	Description: "オープンリダイレクトは、攻撃者が任意の外部ドメインにリダイレクトさせるURLを作成できる脆弱性です。\n" +
		...
		"この攻撃は、正しいドメイン名を利用して証明書のエラーが出ないため被害者が気づきにくくなってしまっており、ログインIDとパスワードなどの個人情報を求められた場合に被害者は入力してしまう可能性があります。\n" +
		"オープンリダイレクトは以下の2点がそろっている場合は脆弱性となりえない場合があります。\n" +
		"・外部ドメインに遷移する仕様である。\n" +
		"・利用者が外部ドメインに遷移することを認知している。\n",
...
var httpheaderiVuln = Vuln{
	...
	Insurance: "外部からの入力の改行コードはすべて削除するようにしましょう。ただし、改行コードを含む文字列を入力された場合はWebアプリケーションが正しく動作しなくなる場合があります。",
	Issues:    []Issue{},
}
